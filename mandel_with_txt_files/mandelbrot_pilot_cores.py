...
def main():
    try:
        ...
        for i in range(1,pdesc.cores+1):
            output_data_list.append('mandel_%d.txt' % i)
        ...
        for i in range(1,pdesc.cores+1):
            data_file = open('mandel_%d.txt' % i,'r')
            read_as_string_array = data_file.readline().split(',')
            ...
            for j in range(0,(len(read_as_int_array))//3):
                x_xbeg = read_as_int_array[3*j]
                y_ybeg = read_as_int_array[3*j+1]
                color = read_as_int_array[3*j+2]
                fullimage.putpixel((x_xbeg, y_ybeg), color)
